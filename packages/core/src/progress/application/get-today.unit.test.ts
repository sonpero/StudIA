...
import { fakeDocumentRepositoryForProgress, fakeNotionRepositoryForProgress, fakeProgressRepository, fakeReviewRepositoryForProgress } from "./fakes.js";
...
describe("getToday", () => {
  it("collects today's entries across every one of the user's documents", async () => {
    const documentRepo = fakeDocumentRepositoryForProgress([aDocument({ id: "doc1" }), aDocument({ id: "doc2" })]);
    const notionRepo = fakeNotionRepositoryForProgress([aNotion({ id: "n1", documentId: "doc1" }), aNotion({ id: "n2", documentId: "doc2" })]);
  ...
  it("silently skips a document whose plan is a ProgressInputError (e.g. no availability set)", async () => {
    const documentRepo = fakeDocumentRepositoryForProgress([aDocument({ id: "doc1" })]);
    const notionRepo = fakeNotionRepositoryForProgress([aNotion({ id: "n1", documentId: "doc1" })]);
    const reviewRepo = fakeReviewRepositoryForProgress([{ userId: "u1", documentId: "doc1", notionId: "n1", masteredCards: 0, totalCards: 1 }]);
    const repo = fakeProgressRepository(); // no availability set -> no-capacity for every document
  ...
  it("only returns entries dated today, not the whole plan", async () => {
    const documentRepo = fakeDocumentRepositoryForProgress([aDocument({ id: "doc1" })]);
    const notionRepo = fakeNotionRepositoryForProgress(
    ...
    const repo = fakeProgressRepository({ availability: { u1: { mon: 20, tue: 20, wed: 20, thu: 20, fri: 20, sat: 20, sun: 20 } } });
