...
import { WEEKDAYS, type Availability, type BuildPlanInput, type Plan, type PlanDay, type PlanEntry, type PlanNotion, type ProgressInputError, type Weekday } from "./types.js";
...
// learn-eligible block and a trailing review-only block (docs/modules/progress.md:
...
function buildBoundedPlan(
  ...
  // smaller one behind it (docs/modules/progress.md's best-effort order).
...
// (docs/modules/progress.md's clustering rule is deadline-relative), so
...
export function buildPlan(input: BuildPlanInput): Result<Plan, ProgressInputError> {
  ...
  // permanently blocking the FIFO queue behind it (docs/modules/progress.md's
  // atomic-notion case).
