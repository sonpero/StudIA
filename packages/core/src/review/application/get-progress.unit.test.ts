...
describe("getProgress", () => {
  it("returns the mastered/total counts and next due date from the repository", async () => {
    ...
    expect(await getProgress({ repo }, "u1", "doc-1", dayBoundary)).toEqual({ mastered: 3, total: 10, nextDueDate: "2026-01-08T00:00:00.000Z" });
    expect(getProgressMock).toHaveBeenCalledWith("u1", "doc-1", dayBoundary);
  });
});
