...
import { fakeNotionRepositoryForPlanning, fakePlanningRepository, fakeReviewRepositoryForPlanning } from "./fakes.js";
...
describe("getPlan", () => {
  it("assembles buildPlan's input from deadline, availability, notions and mastery, and returns its result", async () => {
    const notionRepo = fakeNotionRepositoryForPlanning([aNotion()]);
  ...
  it("excludes a notion whose cards are all mastered", async () => {
    const notionRepo = fakeNotionRepositoryForPlanning([aNotion()]);
    const reviewRepo = fakeReviewRepositoryForPlanning([{ userId: "u1", documentId: "doc1", notionId: "n1", masteredCards: 2, totalCards: 2 }]);
    const repo = fakePlanningRepository({
      deadlines: [{ id: "d1", userId: "u1", documentId: "doc1", date: "2026-03-20", label: null, createdAt: "2026-01-01T00:00:00.000Z" }],
      availability: { u1: { mon: 30, tue: 30, wed: 30, thu: 30, fri: 30, sat: 0, sun: 0 } },
    });
