// In-memory test doubles for planning's own port (CLAUDE.md rule 3), plus
...
export function fakeNotionRepositoryForPlanning(notions: Notion[]): NotionRepository {
  const notImplemented = (method: string) => () => {
    throw new Error(`fakeNotionRepositoryForPlanning: ${method} is not implemented, planning does not call it`);
  };
  return {
...
export function fakeReviewRepositoryForPlanning(progress: (NotionProgress & { userId: string; documentId: string })[]): ReviewRepository {
  const notImplemented = (method: string) => () => {
    throw new Error(`fakeReviewRepositoryForPlanning: ${method} is not implemented, planning does not call it`);
  };
  return {
...
export function fakeDocumentRepositoryForPlanning(documents: Document[]): DocumentRepository {
  const notImplemented = (method: string) => () => {
    throw new Error(`fakeDocumentRepositoryForPlanning: ${method} is not implemented, planning does not call it`);
  };
  return {
