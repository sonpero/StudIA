...
export type PlanningInputErrorKind = "deadline-in-past" | "no-capacity" | "no-usable-day";
...
// docs/modules/planning.md. Modeled as a discriminated result rather than a
// thrown error, so React Query's error state stays reserved for genuine
// network/server failures.
export type PlanResult = { kind: "ok"; plan: Plan } | { kind: "input-error"; error: PlanningInputErrorKind };
...
export async function getPlanForDocument(documentId: string): Promise<PlanResult> {
  ...
  if (res.status === 422) {
    const body = (await res.json()) as { error: PlanningInputErrorKind };
